...
export default function App() {
  ...
  if (flow.stage === "sales" && flow.ticket) {
    return <SalesScreen employee={flow.employee} ticket={flow.ticket} products={flow.products} allProducts={flow.allProducts} salePages={flow.salePages} activePageId={flow.activePageId} categories={flow.categories} query={flow.query} categoryId={flow.categoryId} busy={flow.busy} errorMessage={flow.errorMessage} onDismissError={flow.clearError} onQueryChange={flow.setQuery} onCategoryChange={flow.setCategoryId} onPageChange={flow.setActivePageId} onCreatePage={flow.createSalePage} onPlacePageProduct={(pageId, slotIndex, productId) => void flow.placeSalePageProduct(pageId, slotIndex, productId)} onRemovePageProduct={(pageId, slotIndex) => void flow.removeSalePageProduct(pageId, slotIndex)} onAddProduct={(id) => void flow.addProduct(id)} onSetQuantity={(id, value) => void flow.setQuantity(id, value)} onRemoveLine={(id) => void flow.removeLine(id)} onSaveTicket={() => void flow.saveOpenTicket()} onCheckout={() => void flow.beginCheckout()} />;
  }
