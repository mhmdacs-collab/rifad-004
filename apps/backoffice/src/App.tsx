...
export default function App({ catalog }: { catalog: CatalogAdminContract }) {
  ...
  const selectedOptionGroup = optionGroupEditorId ? optionGroups.find((group) => group.id === optionGroupEditorId) ?? null : null;
  ...
  const selectedPricingGroup = pricing.mode === "option-group" ? optionGroups.find((group) => group.id === pricing.groupId) ?? null : null;
  ...
  const startNewCategory = () => { setCategoryEditorId("new"); setCategoryName(""); setError(null); };
  const startEditCategory = (category: CatalogCategory) => { setCategoryEditorId(category.id); setCategoryName(category.name); setError(null); };
  const saveCategory = async (event: FormEvent) => {
    event.preventDefault(); setSaving(true); setError(null);
    try {
      if (categoryEditorId && categoryEditorId !== "new") await catalog.updateCategory({ commandId: commandId("category-update"), categoryId: categoryEditorId, name: categoryName });
      else await catalog.createCategory({ commandId: commandId("category-create"), name: categoryName });
      await load(); setCategoryEditorId(null); showFlash("تم حفظ الفئة");
    } catch (saveError) { setError(messageFrom(saveError)); } finally { setSaving(false); }
  };

  const openNewOptionGroup = () => { setOptionGroupEditorId(null); setOptionGroupDraft(emptyOptionGroupDraft()); setError(null); };
  const openOptionGroup = (group: CatalogOptionGroup) => { setOptionGroupEditorId(group.id); setOptionGroupDraft({ name: group.name, values: group.values }); setError(null); };
  ...
  const saveOptionGroup = async (event: FormEvent) => {
    ...
    try {
      ...
      await load(); setOptionGroupEditorId(undefined); showFlash("تم حفظ مجموعة الخيارات");
    } catch (saveError) { setError(messageFrom(saveError)); } finally { setSaving(false); }
  };

  const openNewModifier = () => { setModifierEditorId(null); setModifierDraft(emptyModifierDraft()); setError(null); };
  const openModifier = (modifier: CatalogModifierGroup) => { setModifierEditorId(modifier.id); setModifierDraft({ name: modifier.name, options: modifier.options }); setError(null); };
  ...
  const saveModifier = async (event: FormEvent) => {
    ...
    try {
      ...
      await load(); setModifierEditorId(undefined); showFlash("تم حفظ مجموعة الإضافات");
    } catch (saveError) { setError(messageFrom(saveError)); } finally { setSaving(false); }
  };

  const nav = (name: IconName, label: string, active = false, onClick?: () => void, disabled = false) => (
    <button className={`bo-modern-nav ${active ? "bo-modern-nav--active" : ""}`} type="button" onClick={onClick} disabled={disabled}>
  ...
  const renderSidebar = () => (
    <aside className="bo-sidebar bo-sidebar--modern" aria-label="تنقل المكتب الخلفي">
      ...
      <nav className="bo-modern-nav-list">
        {nav("items", "لوحة المعلومات", false, undefined, true)}
        {nav("reports", "التقارير", false, undefined, true)}
        ...
        {nav("inventory", "المخزون", false, undefined, true)}
        {nav("staff", "الموظفون", false, undefined, true)}
        {nav("customers", "العملاء", false, undefined, true)}
        {nav("settings", "الإعدادات", false, undefined, true)}
      </nav>
  ...
  const renderItemsList = () => (
    <>
      ...
      <section className="bo-card bo-catalog-list-card" aria-busy={loading}>
        ...
        <div className="bo-table-wrap"><table className="bo-modern-table"><thead><tr><th>الصنف</th><th>الفئة</th><th>SKU</th><th>السعر</th><th>التسعير</th><th>الإضافات</th><th>الحالة</th></tr></thead><tbody>
          {!loading && visibleItems.length === 0 ? <tr><td colSpan={7} className="bo-empty">لا توجد أصناف مطابقة.</td></tr> : visibleItems.map((item) => {
            ...
            return <tr key={item.id} onClick={() => openItem(item)} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter") openItem(item); }}>
              ...
              <td>{item.categoryName ?? "بدون فئة"}</td><td dir="ltr">{item.sku || "—"}</td>
        ...
        {multiplePrices ? <div className="bo-multi-price-panel">
          ...
          {pricing.mode === "option-group" ? <>
            ...
            {selectedPricingGroup ? <div className="bo-price-options-card">
              <div className="bo-price-options-head"><div><strong>{selectedPricingGroup.name}</strong><small>تتحدث أسعار هذه المجموعة لكل الأصناف التي تستخدمها.</small></div><label className="bo-inline-check"><input type="checkbox" checked={pricing.priceMode === "custom"} onChange={(event) => setGroupPriceMode(event.target.checked)} /><span>تخصيص الأسعار لهذا الصنف</span></label></div>
    ...
    <section className="bo-form-card bo-modern-section">
      ...
      <div className="bo-modern-section-body bo-addon-sections">
        <div><div className="bo-subsection-title"><div><strong>الإضافات العامة</strong><small>أنشئها مرة واربطها بأي عدد من الأصناف.</small></div></div>
          {modifierGroups.length === 0 ? <div className="bo-empty-panel"><strong>لا توجد إضافات عامة</strong><span>أنشئ مجموعة من صفحة الإضافات.</span></div> : <div className="bo-check-list">{modifierGroups.map((modifier) => <label key={modifier.id} className="bo-check-card"><input type="checkbox" checked={(itemDraft.modifierGroupIds ?? []).includes(modifier.id)} onChange={() => toggleModifierForItem(modifier.id)} aria-label={modifier.name} /><span><strong>{modifier.name}</strong><small>{modifier.options.length} خيار</small></span></label>)}</div>}
        </div>
  ...
  const renderItemEditor = () => (
    <form className="bo-editor-page bo-item-editor-2026" onSubmit={saveItem}>
      ...
      <div className="bo-editor-heading"><span>{selectedItem ? "تعديل صنف" : "صنف جديد"}</span><h1>{selectedItem?.name || "إضافة صنف"}</h1></div>
  ...
  const renderCategories = () => <><div className="bo-page-header bo-page-header--compact"><div><h1>الفئات</h1><p>نظّم الأصناف لتسريع الوصول إليها.</p></div><button className="bo-primary bo-action-with-icon" type="button" onClick={startNewCategory}><Icon name="add" />إضافة فئة</button></div><section className="bo-card"><div className="bo-table-wrap"><table className="bo-modern-table bo-simple-table"><thead><tr><th>الفئة</th><th>عدد الأصناف</th></tr></thead><tbody>{categories.map((category) => <tr key={category.id} onClick={() => startEditCategory(category)}><td><div className="bo-category-name"><span className="bo-category-dot" /><strong>{category.name}</strong></div></td><td>{items.filter((item) => item.categoryId === category.id).length}</td></tr>)}</tbody></table></div></section>{categoryEditorId ? <div className="bo-modal-backdrop"><form className="bo-modal bo-focused-modal" onSubmit={saveCategory}><div className="bo-modal-icon"><Icon name="category" size={28} /></div><label className="bo-field"><span>اسم الفئة</span><input aria-label="اسم الفئة" autoFocus value={categoryName} onChange={(event) => setCategoryName(event.target.value)} placeholder="مثال: البيتزا" /></label>{error ? <div className="bo-alert" role="alert">{error}</div> : null}<div className="bo-modal-actions"><button type="button" className="bo-secondary" onClick={() => setCategoryEditorId(null)}>إلغاء</button><button type="submit" className="bo-primary bo-action-with-icon"><Icon name="save" size={16} />حفظ</button></div></form></div> : null}</>;

  const renderOptions = () => <><div className="bo-page-header bo-page-header--compact"><div><h1>مجموعات الخيارات</h1><p>أنشئ المجموعة مرة واحدة واستخدمها مع عشرات أو مئات الأصناف.</p></div><button className="bo-primary bo-action-with-icon" type="button" onClick={openNewOptionGroup}><Icon name="add" />إضافة مجموعة</button></div><section className="bo-card"><div className="bo-table-wrap"><table className="bo-modern-table bo-simple-table"><thead><tr><th>المجموعة</th><th>الخيارات والأسعار</th><th>مستخدمة في</th></tr></thead><tbody>{optionGroups.map((group) => <tr key={group.id} onClick={() => openOptionGroup(group)}><td><div className="bo-row-title-icon"><span className="bo-round-icon"><Icon name="options" size={18} /></span><strong>{group.name}</strong></div></td><td>{group.values.map((value) => `${value.name} ${formatMoney(value.price.halalas)}`).join(" · ")}</td><td>{items.filter((item) => item.pricing?.mode === "option-group" && item.pricing.groupId === group.id).length} صنف</td></tr>)}</tbody></table></div></section>{optionGroupEditorId !== undefined ? <div className="bo-modal-backdrop"><form className="bo-modal bo-modal--wide bo-focused-modal" onSubmit={saveOptionGroup}><div className="bo-modal-icon"><Icon name="options" size={28} /></div><label className="bo-field"><span>اسم المجموعة</span><input aria-label="اسم مجموعة الخيارات" autoFocus value={optionGroupDraft.name} onChange={(event) => setOptionGroupDraft({ ...optionGroupDraft, name: event.target.value })} placeholder="مثال: أحجام البيتزا" /></label><div className="bo-builder-list"><div className="bo-builder-head"><strong>الخيارات والأسعار</strong><span>هذه الأسعار الافتراضية لكل الأصناف المرتبطة بالمجموعة.</span></div>{optionGroupDraft.values.map((value, index) => <div className="bo-builder-row" key={value.id ?? index}><input aria-label={`اسم الخيار ${index + 1}`} value={value.name} onChange={(event) => updateOptionGroupValue(index, { name: event.target.value })} placeholder="مثال: صغير" /><div className="bo-compact-money"><input aria-label={`سعر الخيار ${index + 1}`} type="number" min="0" step="0.01" value={(value.price.halalas / 100).toFixed(2)} onChange={(event) => updateOptionGroupValue(index, { halalas: toHalalas(event.target.value) })} /><span>ر.س</span></div><button type="button" className="bo-icon-danger" disabled={optionGroupDraft.values.length <= 2} onClick={() => setOptionGroupDraft({ ...optionGroupDraft, values: optionGroupDraft.values.filter((_, valueIndex) => valueIndex !== index) })}><Icon name="trash" size={16} /></button></div>)}<button className="bo-add-row" type="button" onClick={addOptionGroupValue}><Icon name="add" size={16} />إضافة خيار</button></div>{error ? <div className="bo-alert" role="alert">{error}</div> : null}<div className="bo-modal-actions"><button type="button" className="bo-secondary" onClick={() => setOptionGroupEditorId(undefined)}>إلغاء</button><button type="submit" className="bo-primary bo-action-with-icon"><Icon name="save" size={16} />حفظ المجموعة</button></div></form></div> : null}</>;

  const renderModifiers = () => <><div className="bo-page-header bo-page-header--compact"><div><h1>الإضافات</h1><p>مجموعات إضافات عامة يمكن ربطها بأي عدد من الأصناف.</p></div><button className="bo-primary bo-action-with-icon" type="button" onClick={openNewModifier}><Icon name="add" />إضافة مجموعة</button></div><section className="bo-card"><div className="bo-table-wrap"><table className="bo-modern-table bo-simple-table"><thead><tr><th>المجموعة</th><th>الإضافات</th><th>مستخدمة في</th></tr></thead><tbody>{modifierGroups.map((modifier) => <tr key={modifier.id} onClick={() => openModifier(modifier)}><td><div className="bo-row-title-icon"><span className="bo-round-icon"><Icon name="addon" size={18} /></span><strong>{modifier.name}</strong></div></td><td>{modifier.options.map((option) => `${option.name} +${formatMoney(option.price.halalas)}`).join(" · ")}</td><td>{items.filter((item) => (item.modifierGroupIds ?? []).includes(modifier.id)).length} صنف</td></tr>)}</tbody></table></div></section>{modifierEditorId !== undefined ? <div className="bo-modal-backdrop"><form className="bo-modal bo-modal--wide bo-focused-modal" onSubmit={saveModifier}><div className="bo-modal-icon"><Icon name="addon" size={28} /></div><label className="bo-field"><span>اسم المجموعة</span><input aria-label="اسم مجموعة الإضافات" autoFocus value={modifierDraft.name} onChange={(event) => setModifierDraft({ ...modifierDraft, name: event.target.value })} placeholder="مثال: إضافات القهوة" /></label><div className="bo-builder-list"><div className="bo-builder-head"><strong>الإضافات</strong><span>السعر هنا يضاف إلى سعر الصنف.</span></div>{modifierDraft.options.map((option, index) => <div className="bo-builder-row" key={option.id ?? index}><input aria-label={`اسم الإضافة ${index + 1}`} value={option.name} onChange={(event) => updateModifierOption(index, { name: event.target.value })} placeholder="مثال: شوت إضافي" /><div className="bo-compact-money"><input aria-label={`سعر الإضافة ${index + 1}`} type="number" min="0" step="0.01" value={(option.price.halalas / 100).toFixed(2)} onChange={(event) => updateModifierOption(index, { halalas: toHalalas(event.target.value) })} /><span>ر.س</span></div><button type="button" className="bo-icon-danger" disabled={modifierDraft.options.length <= 1} onClick={() => setModifierDraft({ ...modifierDraft, options: modifierDraft.options.filter((_, optionIndex) => optionIndex !== index) })}><Icon name="trash" size={16} /></button></div>)}<button className="bo-add-row" type="button" onClick={addModifierOption}><Icon name="add" size={16} />إضافة خيار</button></div>{error ? <div className="bo-alert" role="alert">{error}</div> : null}<div className="bo-modal-actions"><button type="button" className="bo-secondary" onClick={() => setModifierEditorId(undefined)}>إلغاء</button><button type="submit" className="bo-primary bo-action-with-icon"><Icon name="save" size={16} />حفظ المجموعة</button></div></form></div> : null}</>;

  return <div className="bo-shell" dir="rtl">{renderSidebar()}<main className="bo-main"><div className="bo-topbar"><div className="bo-topbar-title"><strong>{itemEditorId !== undefined ? "الصنف" : page === "categories" ? "الفئات" : page === "options" ? "مجموعات الخيارات" : page === "modifiers" ? "الإضافات" : "قائمة الأصناف"}</strong><span>المتجر الرئيسي</span></div><span className="bo-top-status">تجربة محلية</span></div><div className="bo-content">{itemEditorId !== undefined ? renderItemEditor() : page === "categories" ? renderCategories() : page === "options" ? renderOptions() : page === "modifiers" ? renderModifiers() : renderItemsList()}</div></main>{flash ? <div className="bo-toast" role="status">✓ {flash}</div> : null}</div>;
}
