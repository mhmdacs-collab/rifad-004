  ...
  try {
    ...
    if (req.method === 'POST' && req.url === '/sync-batch') {
      ...
      if (containsSale && failAfterApplyingNextSale) {
        ...
        json(res, 503, { applied: true, acknowledged: false, reason: 'intentional ambiguous browser failure proof' });
        return;
      }
      json(res, 200, { ok: true });
      return;
    ...
    if (req.method === 'POST' && req.url === '/control/item') {
      ...
      await pool.query(
        `INSERT INTO catalog_items(id, merchant_id, branch_id, name, price_minor, metadata_json)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, price_minor = EXCLUDED.price_minor, metadata_json = EXCLUDED.metadata_json`,
        [item.id, 'merchant-proof', 'branch-proof', item.name, item.priceMinor, JSON.stringify(item.metadata ?? {})]
      );
      json(res, 200, { ok: true });
      return;
    }
    ...
    if (req.method === 'GET' && req.url?.startsWith('/control/dedupe-count')) {
      const url = new URL(req.url, `http://127.0.0.1:${PORT}`);
      const device = url.searchParams.get('device') ?? '';
      const result = await pool.query('SELECT COUNT(*)::int AS count FROM sync_upload_dedupe WHERE device_id = $1', [device]);
      json(res, 200, { count: result.rows[0].count });
      return;
    }
...
server.listen(PORT, '127.0.0.1', () => {
  console.log(`RIFAD_WEB_PROOF_BACKEND_READY port=${PORT}`);
});
